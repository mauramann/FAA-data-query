...
top50 |> ggplot(aes(x=Origin, y=Avg_Delay)) +
  ...
  annotate("text", x=grep("OTH", top50$Origin)*1.75, y=top50$Avg_Delay[which(top50$Origin=="OTH")]*1.01, label="Barkley Regional", color="orange", size=2 , angle=0, fontface="bold", hjust=0)
```
...
back50 |> ggplot(aes(x=Origin, y=Avg_Delay)) +
  ...
  theme(axis.text.x = element_text(size = 7)) +
  annotate("text", x=grep("AKN", back50$Origin), y=back50$Avg_Delay[which(back50$Origin=="AKN")]*1.2, label="King Salmon Airport", color="orange", size=2.5 , angle=0, fontface="bold", hjust=0)
```
